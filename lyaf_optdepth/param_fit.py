...
def fit_alpha(flux, ivar, redshift=-1, wl=wl, wav_range=None,
    ...
    if wav_range is None:
        wav_range = np.array([[1280, 1290], [1317, 1325],
                             [1350, 1360], [1440, 1470]])
    ...
    try:
        ...
        if plotit:
            ...
            ax.plot(wl, alpha_func(wl, result['x']), lw=0.6, c='r',
                    label=r'$\alpha = %.2f, \chi^2_r = %.2f, \
                    \sigma_\alpha = %.2f$' %(result['x'][1], 2 * chisq_r, alpha_sig))
...
def fit_EW(flux, ivar, redshift=-1, wl=wl, plotit=False,
    ...
    try:
        popt, __ = curve_fit(lin_funct, wave, spectra, sigma=error)

...
def main():
    ...
    data = fitsio.read(config.drqcat, ext=int(config.drq_dataext))
    plate, mjd, fiberid = data['PLATE'], data['MJD'], data['FIBERID']

    # Which redshift to use?
    z_use = data[config.drq_z]
    ...
    ind = np.where((balFlag == 0) & (data['ZWARNING'] == 0) &
                   (z_use > float(config.zmin)))[0]
    # --------------------------------------------------------------------------

    # 2b. Remove DLA's - Ref: (Noterdaeme et al. 2012c)
    # http://adsabs.harvard.edu/abs/2012A%26A...547L...1N
    dla_data = np.loadtxt(config.dla_file, skiprows=2, usecols=([1]), dtype='str').T
    ...
    if config.run != 0:
        ind = np.random.choice(ind, size=config.run, replace=False)

    # --------------------------------------------------------------------------
    # Load in the directory which contain the spectra
    spec_dir = config.spec_dir

    # Load in the sky-lines file - There are a lot of skylines!!!
    skylines = np.loadtxt(config.skyline_file).T

    # These will be written to the final catalogue
    calFlux, calInvar = np.zeros((len(ind), config.NPIX)), np.zeros((len(ind), config.NPIX))
    ...
    for k in range(nObj):
        ...
        try:
            ...
            s = np.floor((log2rest - config.COEFF0)/config.COEFF1).astype(int)
            temp1 = np.where((s >= 0) & (s < config.NPIX))[0]
            ...
            minT = np.floor((minSky - config.COEFF0)/config.COEFF1).astype(int)
            maxT = np.ceil((maxSky - config.COEFF0)/config.COEFF1).astype(int)
    ...
    mydict = OrderedDict()
    keys = config.paramstowrite.split(', ')
    for key in keys:
        mydict[key] = data[key][ind]

    # Write images and table to fits file
    hdict = {'COEFF0': config.COEFF0, 'COEFF1': config.COEFF1, 'NPIX': config.NPIX,
